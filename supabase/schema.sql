...
create table if not exists prints (
  number text primary key, -- e.g. "55", "123-A"
  title text not null,
  description text,
...
create policy "Allow public read access on interpellations" on interpellations for select using (true);
create policy "Allow public read access on prints" on prints for select using (true);
