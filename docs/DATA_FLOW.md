...
- [x] **Jasny Motyw (Light Mode)**: Zaprojektowanie i wdrożenie profesjonalnego, czytelnego trybu jasnego (obecnie dominuje ciemny).
- [x] **Optymalizacja Zdjęć Posłów**: Zmniejszenie rozmiaru/wagi zdjęć w sekcji "Posłowie" (poprawa rozdzielczości/wydajności poprzez gęstszą siatkę 8-kolumnową i lazy loading).
- [x] **Teksty "O Projekcie"**: Przeredagowanie sekcji "O projekcie". Nowy tekst ma być:
...
- [x] **Archiwum Wypowiedzi**:
    - Wyświetlanie *ostatnich* wypowiedzi z mównicy.
    - Możliwość przejścia do pełnej listy wszystkich stenogramów danego posła.
- [x] **Kontakt**: Dodanie oficjalnych danych kontaktowych (biuro poselskie, email, social media posła).

## 🗳️ Funkcje: Edukacja i Dane

- [x] **Test Wyborczy (Latarnik)**: Kompleksowe ulepszenie algorytmu i pytań w teście dopasowania politycznego (dynamiczne dane, oficjalne logotypy).
...
- [x] **Backend Wyszukiwarki**: Implementacja logiki przeszukiwania wielu tabel jednocześnie (głosowania, ustawy, posłowie, interpelacje).
- [x] **Filtrowanie**: "Zaprzęgnięcie do działania" istniejących filtrów UI:
    - *Ostatni tydzień*.
    - *Tylko ustawy*.
    - *Kontrowersyjne* (close votes).
- [x] **Kontekstowość**: Wyszukiwarka ma "rozumieć" synonimy i kontekst (np. "szpitale" -> szuka też w ustawach zdrowotnych).

## 🖼️ UI: Panel Głosowania (`VoteDetails`)

Kosmetyka i UX w widoku konkretnego głosowania.

- [x] **Powiązany Druk Sejmowy**:
    - Przesunięcie i zmniejszenie segmentu (Wdrożono wersję Prominentną).
    - Lepsza integracja wizualna z resztą strony.
- [x] **Mapa Myśli (Mind Map)**:
    - [x] Poprawa layoutu (Wizualizacja argumentów Za/Przeciw).
    - [x] Stylizacja przycisku "Zobacz mapę myśli", aby pasował do designu strony.
