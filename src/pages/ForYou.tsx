...
import { Briefcase, GraduationCap, Tractor, User, Users, Baby, ChevronRight, ArrowLeft, Gavel, FileText } from 'lucide-react';
...
export default function ForYou() {
    ...
    const { data: personaVotes, isLoading: isPersonaLoading } = usePersonasFeed(selectedPersona);
                ...
                <div className="container mx-auto px-4">
                    <div className="max-w-3xl mx-auto text-center mb-16">
                        ...
                        <p className="text-xl text-text-secondary leading-relaxed">
                            Nie trać czasu na czytanie tysięcy stron ustaw. Wybierz kim jesteś, a AI wyciągnie to, co realnie wpływa na Twoje życie.
                        </p>
                    </div>
                        ...
                        {PERSONAS.map((persona) => {
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <div className={`w-14 h-14 rounded-2xl flex items-center justify-center mb-6 text-white shadow-lg bg-gradient-to-br ${persona.color}`}>
                                        <Icon size={28} />
                                    </div>
                                    <h3 className="text-2xl font-bold mb-2 group-hover:text-primary transition-colors">{persona.label}</h3>
                                    <p className="text-text-secondary group-hover:text-text-primary transition-colors">{persona.desc}</p>
    ...
    return (
        <div className="min-h-screen bg-page text-text-primary pb-20 pt-24">
                    ...
                    {PERSONAS.map((persona) => {
                        ...
                        return (
                            <button
                                key={persona.id}
                                onClick={() => setSelectedPersona(persona.id)}
                                className={`flex items-center gap-2 px-4 py-2 rounded-xl transition-all duration-300 text-sm font-bold ${isActive ? `bg-gradient-to-r ${persona.color} text-white shadow-lg` : 'bg-card-bg border border-border-base hover:bg-card-hover text-text-secondary'}`}
                            >
                                <Icon size={16} />
                                {persona.label}
                            </button>
            ...
            <div className="container mx-auto px-4 grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                            ...
                            >
                                ...
                                <div className="relative mb-4">
                                    <div className="flex justify-between items-start gap-4 mb-3">
                                        ...
                                        <div className={`flex items-center justify-center px-2 py-1 rounded-full text-xs font-bold gap-1 ${vote.importance >= 8 ? 'bg-red-500/20 text-red-400' : 'bg-primary/10 text-primary'}`}>
                                            <span>Waga:</span>
                                            <span>{vote.importance}/10</span>
                                        </div>
                                    </div>
                                    <h3 className="text-lg font-bold leading-tight group-hover:text-primary transition-colors line-clamp-3">{vote.title}</h3>
                                </div>
                                <div className="mt-auto">
                                    <div className="relative p-4 rounded-2xl border border-white/5 bg-white/5 group-hover:bg-white/10 transition-colors">
                                        ...
                                        <p className="text-sm font-medium leading-relaxed text-text-primary/90">{vote.impact_text}</p>
                                    </div>
                                </div>
                ...
                {!isPersonaLoading && (!personaVotes || personaVotes.length === 0) && (
