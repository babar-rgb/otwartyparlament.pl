...
export default function CategoryDetails() {
    ...
    // Filter votes from shared mock data
    const votes = slug
        ? MOCK_VOTES.filter(v => v.categorySlug === slug.toLowerCase())
        : MOCK_VOTES;

    const stats = {
        total: votes.length,
        accepted: votes.filter(v => v.verdict === 'PRZYJĘTO').length,
        rejected: votes.filter(v => v.verdict === 'ODRZUCONO').length,
    };
                    ...
                    {votes.map((vote, index) => (
                        <Link
                            ...
                            state={{
                                vote: {
                                    ...
                                    yes: vote.votesYes,
                                    no: vote.votesNo,
                                    abstain: vote.votesAbstain,
                                    kind: vote.kind,
                                    topic: vote.categoryLabel,
                        ...
                        >
                            <VoteItem
                                ...
                                date={vote.date}
                                result={vote.verdict}
                                votesYes={vote.votesYes}
                                votesNo={vote.votesNo}
                            />
                        </Link>
                    ))}
                </div>
