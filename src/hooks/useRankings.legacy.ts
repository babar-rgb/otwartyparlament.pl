...
export function useRankings() {
    ...
    useEffect(() => {
        const load = async () => {
            try {
                const mpData = await fetchMPs({ active: true, limit: 500 });
                setMps(mpData.map((m: any) => ({
                    id: m.id,
                ...
                const { items: processes } = await fetchProcesses({ limit: 100 });
                ...
                console.error('Error in useRankings:', err);
            } finally {
                setLoading(false);
            }
        };
