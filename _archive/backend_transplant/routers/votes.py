...
def read_votes(
    ...
    grouped: bool = False,
    sort_by: Optional[str] = None,  # 'importance' | 'date' (default)
    db: Session = Depends(database.get_db)
):
    ...
    if hide_procedural and category != 'PROCEDURAL':
        ...
        # Fix: use SQLAlchemy ~ (NOT) operator, not Python's 'not'
        ...
        # Exclude flagged procedural OR title-matched procedural
        procedural_conditions = [models.Vote.title_raw.ilike(p) for p in procedural_title_patterns]
        if hasattr(models.Vote, 'is_procedural') and models.Vote.is_procedural is not None:
            procedural_conditions.append(models.Vote.is_procedural == True)
        query = query.filter(not_(or_(*procedural_conditions)))
    if grouped:
    ...
    # --- ORDERING ---
    if sort_by == 'importance' and hasattr(models.Vote, 'importance'):
        from sqlalchemy import nullslast
        query = query.order_by(
            nullslast(models.Vote.importance.desc()),
            models.Vote.date.desc()
        )
    else:
        query = query.order_by(models.Vote.date.desc(), models.Vote.voting_number.desc())
