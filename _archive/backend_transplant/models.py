from sqlalchemy import Column, Integer, String, Date, ForeignKey, Boolean, Text, Float, DateTime, BigInteger, text
...
class MP(Base):
    ...
    club_history = Column(JSONB, nullable=True)
    ...
    declarations = Column(Text, nullable=True)
...
class Vote(Base):
    ...
    ai_summary = Column(Text, nullable=True) # Citizen
    expert_summary = Column(Text, nullable=True) # Detailed
    kind = Column(String, nullable=True) # e.g., 'ustawa', 'uchwala'
    ...
    # Restored Metadata (Added Jan 2026 Blitz)
    category = Column(Text, nullable=True)
    importance_score = Column(Float, default=0.0)
    is_key_vote = Column(Boolean, default=False)
    is_cluster_primary = Column(Boolean, default=False)
    cluster_id = Column(Integer, nullable=True)
    semantic_weight = Column(Float, nullable=True)
    controversy_score = Column(Float, nullable=True)
    persona_tags = Column(ARRAY(Text), nullable=True)
    title_short = Column(Text, nullable=True)
    description = Column(Text, nullable=True)
    name_citizen = Column(Text, nullable=True)
    topic_tag = Column(Text, nullable=True)
    ux_category = Column(Text, nullable=True)
    ...
    # Analysis / Personas (Added Jan 2026 Blitz)
    pros = Column(JSONB, nullable=True) # AI generated
...
class VoteResult(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, server_default=text("extensions.uuid_generate_v4()"))
    ...
    rebel = Column(Boolean, default=False)
    created_at = Column(DateTime, server_default=func.now())
...
class Speech(Base):
    ...
    related_vote_id = Column(Integer, ForeignKey("votes.id"), nullable=True)
    created_at = Column(DateTime, server_default=func.now())
    
    vote = relationship("Vote", primaryjoin="Speech.related_vote_id == Vote.id", backref="speeches")
...
class LegislativeStage(Base):
    ...
    procedural_context = Column(Text, nullable=True)
    legal_consequence = Column(Text, nullable=True)
...
class AnalysisRequest(Base):
    ...
    target_type = Column(String, index=True) # 'vote', 'bill', 'interpellation'
    target_id = Column(String, index=True) # ID of the object
    status = Column(String, index=True, default='PENDING') # PENDING, PROCESSING, COMPLETED, FAILED, RETRY
...
class SystemHealth(Base):
    """
    Tracks the health and status of background processes (Heartbeat).
    """
    __tablename__ = "system_health"
    
    id = Column(String, primary_key=True) # e.g. 'orchestrator'
    status = Column(String, default='HEALTHY')
    last_heartbeat = Column(DateTime, server_default=func.now())
    version = Column(String, nullable=True)
    
    # Metrics stored as JSON for flexibility
    # Using JSONB from postgresql for production, mapping to TEXT or JSON in models
    from sqlalchemy.dialects.postgresql import JSONB
    metrics = Column(JSONB, nullable=True) # { "tasks_completed": 100, "avg_time_ms": 1200 }
    
    updated_at = Column(DateTime, server_default=func.now(), onupdate=func.now())


# --- Meta-Topics (Cross-cutting thematic tags) ---

class MetaTopic(Base):
    """
    Represents a cross-cutting 'Meta Topic' that groups votes and interpellations
    across sessions/prints. E.g. #KSeF, #Aborcja, #RentaWdowia.
    """
    __tablename__ = "meta_topics"
    ...
    heat = Column(Integer, default=50)  # trending score 0-100
    category = Column(String, nullable=True)  # e.g. 'podatki', 'zdrowie'
    status = Column(String, default="W toku")  # W toku, Przyjęto, Odrzucono, Oczekuje
    key_politicians = Column(JSONB, nullable=True)  # [{name, role, party}]
    party_stances = Column(JSONB, nullable=True)  # [{party, for_pct, against_pct, abstain_pct, total}]
    timeline = Column(JSONB, nullable=True)  # [{date, event, type}]
    created_at = Column(DateTime, server_default=func.now())
    updated_at = Column(DateTime, server_default=func.now(), onupdate=func.now())


class VoteMetaTopic(Base):
    """Junction table: Vote <-> MetaTopic (many-to-many)."""
    __tablename__ = "vote_meta_topics"

    vote_id = Column(Integer, ForeignKey("votes.id"), primary_key=True)
    meta_topic_id = Column(Integer, ForeignKey("meta_topics.id"), primary_key=True)


class InterpellationMetaTopic(Base):
    """Junction table: Interpellation <-> MetaTopic (many-to-many)."""
    __tablename__ = "interpellation_meta_topics"
