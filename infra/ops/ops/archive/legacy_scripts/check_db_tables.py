...
try:
    # List tables (simulated by trying to select from likely names)
    tables = ['individual_votes', 'mp_votes', 'votes']
    for t in tables:
        try:
            response = supabase.table(t).select("*").limit(1).execute()
            print(f"Table '{t}' exists. Columns: {response.data[0].keys() if response.data else 'Empty'}")
